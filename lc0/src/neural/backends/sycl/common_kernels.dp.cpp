...
// Use kInputPlanes from sycl_common.h - avoid duplicate definition
      ...
      queue.submit([&](sycl::handler& h) {
        h.parallel_for(sycl::nd_range<3>(global_range, local_range),
          [=](sycl::nd_item<3> item) {
            ...
            (void)fp16; // Suppress unused variable warning for now
...
void globalAvgPool_kernel(sycl::queue& queue, T* output, const T* input,
  ...
  (void)prevLayerBias; // Suppress unused parameter warning
...
void globalAvgPool(sycl::queue& queue, int N, int C, T* output, const T* input,
  ...
  if (nhwc) {
    ...
    globalAvgPool_NHWC_fp16_kernel<sycl::half>(queue,
    ...
    (void)kWorkGroupSize; // Suppress unused variable warning
    (void)blocks; // Suppress unused variable warning
  ...
  queue.submit([&](sycl::handler& h) {
    ...
    h.parallel_for(sycl::nd_range<2>(global_range, local_range),
      [=](sycl::nd_item<2> item) {
        ...
        float subgroup_max_result = sycl::reduce_over_group(sg, x, sycl::maximum<float>());
        ...
        if (sg.get_local_id()[0] == 0) {
          ...
          do {
            expected = current;
            current = max_ref.compare_exchange_strong(expected, subgroup_max_result > expected ? subgroup_max_result : expected);
