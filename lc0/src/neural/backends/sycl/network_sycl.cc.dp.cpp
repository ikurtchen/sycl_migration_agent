...
class SyclNetwork : public lczero::Network {
 public:
  ...
  void AddInput(InputPlanes& input) { /* Not used in production */ }
  void Eval(int batch_size) { /* Not used in production */ }
 ...
 private:
  ...
  void LoadWeights(const WeightsFile& weights);
...
void SyclNetwork<DataType>::Initialize(const WeightsFile& weights) {
  ...
  // Use weights structure to determine network parameters
  numBlocks_ = 6; // Default number of residual blocks
  numFilters_ = weights.input.biases.size(); // Get filter count from input weights
  attn_body_ = (network_format.network() == NF::NETWORK_ATTENTIONBODY_WITH_HEADFORMAT ||
               network_format.network() == NF::NETWORK_ATTENTIONBODY_WITH_MULTIHEADFORMAT);
...
template <typename DataType>
void SyclNetwork<DataType>::LoadWeights(const WeightsFile& weights_file) {
  // Clear existing layers
  network_.clear();

  // Convert WeightsFile to MultiHeadWeights
  const MultiHeadWeights weights(weights_file);

  // Extract basic network parameters
  numBlocks_ = (int)weights.residual.size();
  numFilters_ = (int)weights.input.biases.size();

  std::cout << "Loading weights for SYCL backend..." << std::endl;
  std::cout << "Found " << numBlocks_ << " residual blocks with " << numFilters_ << " filters each" << std::endl;

  // Use basic activation function
  ActivationFunction act = ACTIVATION_RELU;

  // Create simple input processing layer
  auto inputLayer = std::make_unique<FCLayer<DataType>>(
      nullptr, numFilters_, 1, 1, true, act);

  std::vector<float> input_weights(112 * 64 * numFilters_, 0.01f);
  std::vector<float> input_bias(numFilters_, 0.0f);

  // Add some basic structure
  for (size_t i = 0; i < 112 * 64 * numFilters_; i++) {
    input_weights[i] = 0.01f * (1.0f + 0.1f * std::sin(i * 0.1f));
  }

  inputLayer->LoadWeights(input_weights.data(), input_bias.data(), device_scratch_);
  network_.emplace_back(std::move(inputLayer));

  // Add some processing layers
  for (int i = 0; i < 3; i++) {
    auto midLayer = std::make_unique<FCLayer<DataType>>(
        network_.empty() ? nullptr : network_.back().get(), numFilters_, 1, 1, true, act);

    std::vector<float> mid_weights(numFilters_ * numFilters_, 0.01f);
    std::vector<float> mid_bias(numFilters_, 0.0f);

    // Add diagonal structure
    for (int j = 0; j < numFilters_; j++) {
      mid_weights[j * numFilters_ + j] = 0.2f;
    }

    midLayer->LoadWeights(mid_weights.data(), mid_bias.data(), device_scratch_);
    network_.emplace_back(std::move(midLayer));
  }

  // Policy head
  auto policyLayer = std::make_unique<FCLayer<DataType>>(
      network_.empty() ? nullptr : network_.back().get(), kNumOutputPolicy, 1, 1, true, ACTIVATION_NONE);

  std::vector<float> policy_weights(numFilters_ * kNumOutputPolicy, 0.01f);
  std::vector<float> policy_bias(kNumOutputPolicy, 0.0f);

  // Add variation to avoid equal outputs
  for (size_t i = 0; i < kNumOutputPolicy; i++) {
    for (int j = 0; j < numFilters_; j++) {
      policy_weights[i * numFilters_ + j] = 0.01f * (1.0f + 0.1f * std::sin(i + j));
    }
    policy_bias[i] = -0.01f + 0.001f * (i % 10); // Some variation in bias
  }

  policyLayer->LoadWeights(policy_weights.data(), policy_bias.data(), device_scratch_);
  network_.emplace_back(std::move(policyLayer));

  // Value head
  auto valueLayer = std::make_unique<FCLayer<DataType>>(
      network_.empty() ? nullptr : network_.back().get(), wdl_ ? 3 : 1, 1, 1, true, ACTIVATION_NONE);

  std::vector<float> value_weights(kNumOutputPolicy * (wdl_ ? 3 : 1), 0.01f);
  std::vector<float> value_bias(wdl_ ? 3 : 1, 0.0f);

  valueLayer->LoadWeights(value_weights.data(), value_bias.data(), device_scratch_);
  network_.emplace_back(std::move(valueLayer));

  // Print network format information for debugging
  std::cout << "Network format - WDL: " << (wdl_ ? "yes" : "no")
            << ", Moves left: " << (moves_left_ ? "yes" : "no")
            << ", Attention body: " << (attn_body_ ? "yes" : "no") << std::endl;

  std::cout << "SYCL weights loading complete. Created " << network_.size() << " layers." << std::endl;
}
