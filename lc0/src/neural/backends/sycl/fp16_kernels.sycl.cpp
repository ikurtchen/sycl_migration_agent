  ...
  queue.submit([&](sycl::handler& cgh) {
    ...
    cgh.parallel_for(sycl::nd_range<1>{
      ...
      for (int h = 0; h < 8; h++) {
        ...
        if (use_skip) {
          for (int w = 0; w < 8; w++) {
            BOARD(h, w) += skip[INDEX_NHCW(n, k, h, w)];
          }
        }
      ...
      for (int y = 0; y < 6; y++) {
        for (int x = 0; x < 6; x++) {
          ...
          output[TEMP_INDEX_HWNC(y, x, n * 4 + 0, k)] = sycl::half(0.0f); // placeholder
          output[TEMP_INDEX_HWNC(y, x, n * 4 + 1, k)] = sycl::half(0.0f); // placeholder
          output[TEMP_INDEX_HWNC(y, x, n * 4 + 2, k)] = sycl::half(0.0f); // placeholder
          output[TEMP_INDEX_HWNC(y, x, n * 4 + 3, k)] = sycl::half(0.0f); // placeholder
        }
      }
    });
  });
